...
// .w   = random value (0-1)
//
Shader "Hidden/Kvant/Wall/TunnelKernel"
{
    CGINCLUDE

    #include "UnityCG.cginc"
    #include "../Kvant/Wall/Shaders/SimplexNoiseExt3D.cginc"
    ...
    float4 frag_position(v2f_img i) : SV_Target
    {
		...
		float3 pos = origin + disp;
				
		float theta = i.uv.x * UNITY_PI * 2;
		float r = pos.z + _Extent.x;
    ...
    float4 frag_rotation(v2f_img i) : SV_Target
    {
		...
		float3 axis0 = float3(0,1,0);

		float4 q0 = float4(axis0 * sin(theta), cos(theta));
		float4 q1 = float4(axis  * sin(angle), cos(angle));
        return qmul(q0, q1);
    }
